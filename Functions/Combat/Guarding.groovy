...
//	We use ``LivingHurtEvent`` because Damage cannot be modified in ``LivingAttackEvent``.
eventManager.listen
{
	LivingHurtEvent event ->
	{
		...
		float damageBlockable = damage * 0.25f;
		float damageUnblockable = damage * 0.50f;
...
//	We use ``LivingAttackEvent`` because it's called right before Damage is received.
/*
eventManager.listen
{
	LivingAttackEvent event ->
	{
		...
		if (!defenderItem.isEmpty() && defenderItem.getItem() instanceof ItemSword)
		{
			defenderHasSword = true;
		}
		if (!defenderItem.isEmpty() && defenderItem.getItem() instanceof ItemAxe)
		{
			defenderHasAxe = true;
		}
